...
class Rfdc(pr.Device):
    def __init__(
        ...
        self.add(pr.RemoteVariable(
            name         = 'ClkDistMap',
            description  = 'Cached clock distribution map, four bits per tile, ADC 0 in bits 3:0 through DAC 3 in bits 31:28. A nibble reads 0xF when the tile is ungrouped, and otherwise the tile index (tile type times four plus tile id) of the master that tile takes its clock from. A nibble naming a master always names a tile whose own nibble equals its own index, which is an invariant of the cache rather than an accident of a decode, so a word violating it indicates a driver older than this one. A tile whose master chain could not be resolved to a marked master, whether because the chain contains a cycle or because it names a master index out of range, has its grouping withdrawn and reads the ungrouped nibble instead, and one report on the error channel names every tile that was withdrawn. A word reading every tile ungrouped therefore has two readings an operator has to tell apart: on a board with no clock distribution it is the expected reading, and on a board that has one it means the grouping was withdrawn and the report says which tiles',
            offset       = 0x12014,
            bitSize      = 32,
            mode         = 'RO',
            disp         = '{:#010x}',
